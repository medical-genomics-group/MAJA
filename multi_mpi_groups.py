...
def main(n, p, q, iters, burnin, groups, itc, xfile, yfile, dir, gindex, diagnostics):
    ...
    beta = None
    tracker = None
    #Z_sum = np.ones(G, dtype='i')*p_split
    ...
    for it in trange(iters, desc="Main loop"):
        ...
        if rank==0:
            #tracker = -np.ones(p_split*worldSize)
            logger.info(f"{rank=}, {tracker=}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--x', type=str, help='directory in which genotype matrix in npz_stack files is stored', required = True)
